                ...
                child: Column(
                  children: [
                    TextFormField(
                      ...
                      initialValue: widget.isFromEdit ? widget.movie['name'] : '',
                      onChanged: (val) {
                        _name = val;
                      },
                    ),
                    TextFormField(
                      ...
                      initialValue: widget.isFromEdit ? widget.movie['poster'] : '',
                    ),
                    TextFormField(
                      decoration: InputDecoration(
                      ...
                      initialValue: widget.isFromEdit ? widget.movie['length'] : '',
                    ),
                    Padding(
                          ...
                          if (_formKey.currentState.validate()) {
                            ...
                            try {
                              if (!widget.isFromEdit) {
                                ...
                                _name = _name != '' ? _name : widget.movie['name'];
                                _posterURL = _posterURL != '' ? _posterURL : widget.movie['poster'];
                                _length = _length != '' ? _length : widget.movie['length'];
                                Movie _m = new Movie(_name, _posterURL, _length);
                              ...
                              final successSnackbar = SnackBar(
                                content: Text(widget.isFromEdit ? 'Edited Successfully !' : 'Added Successfully !'),
                                duration: Duration(seconds: 2),
                              );
                              _keyScaffold.currentState.showSnackBar(successSnackbar).closed.then((data) => {Navigator.pop(context)});
                            } catch (e) {
                              setState(() {
                                _isLoading = false;
                                _error = e.message;
                              });
                              final failureSnackbar = SnackBar(content: Text('Error : $_error'));
                              _keyScaffold.currentState.showSnackBar(failureSnackbar);
                            }
